...
| T06 | Pipeline 1: Baseline RAG (BM25) | Anish | CODE DONE / RE-RUN PENDING | `src/pipeline_baseline.py`: full-corpus BM25 top-3, schema-conformant output + config sidecar. **⚠️ committed run used local llama3.2, not the agreed GPT-3.5-turbo — re-running on `--backend openai` (now default) over all 150.** | 2026-07-03 | |
...
| T08 | RAGAS eval harness (baseline) | Anish | CODE DONE / RE-RUN PENDING | `src/evaluate.py`: pipeline-agnostic RAGAS (faithfulness/answer_relevancy/context_precision) + MiniLM embeddings, mean/std + per-row. **⚠️ committed scores used a local llama3.2 judge on a 50-q subset — re-running with the GPT-3.5 judge over all 150.** | 2026-07-03 | |
| T09 | QA metrics (F1 + EM) | Anish | DONE (code) | `src/qa_metrics.py`: SQuAD-normalized token F1 + EM, abstention-aware. Code is model-agnostic and correct; **committed numbers (EM 0.00, F1 0.11) are from the llama3.2 run and auto-refresh after the GPT-3.5 baseline re-run.** | 2026-07-03 | 2026-07-03 |
| T10 | Preliminary results table | Anish | CODE DONE / RE-RUN PENDING | `src/build_results_table.py` merges RAGAS + F1/EM -> `results/preliminary_results.md` + `results/results_table.tex` (Markdown + LaTeX) with interpretation. **⚠️ regenerate after the GPT-3.5 baseline + RAGAS re-runs.** | 2026-07-03 | |
