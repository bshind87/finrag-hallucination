...
| T05 | Preprocessing & chunking | Anish + Bhalchandra | DONE | `src/preprocess.py`: two strategies (fixed_512 + sentence), tiktoken token counts, per-chunk metadata (company/doc_type/page range/chunk_id) + `load_chunks()`. Rebuilt 21,876 fixed_512 chunks over 84 PDFs -> `data/processed/*.parquet` (gitignored) | 2026-07-03 | 2026-07-03 |
| T06 | Pipeline 1: Baseline RAG (BM25) | Anish + Bhalchandra | DONE | `src/pipeline_baseline.py`: full-corpus BM25 top-3 + **GPT-3.5-turbo** (temp 0), schema-conformant output + config sidecar. Ran all 150 on OpenAI: 42.7% correct-filing retrieval, 47/150 answered (~69% abstain); `results/baseline_run_summary.md` | 2026-07-03 | 2026-07-03 |
| T07 | Freeze output schema + run config | Anish + Bhalchandra | DONE | `src/schema.py` (REQUIRED_FIELDS, RunConfig, validate_row, read/write helpers) + `src/SCHEMA.md`; RAGAS field map + per-run sidecar `.config.json` documented | 2026-07-03 | 2026-07-03 |
| T08 | RAGAS eval harness (baseline) | Anish + Bhalchandra | DONE | `src/evaluate.py`: pipeline-agnostic RAGAS + MiniLM embeddings, mean/std + per-row. **GPT-3.5 judge over all 150:** faithfulness 0.10, ans-rel 0.21, ctx-prec 0.53 -> `results/eval_ragas.csv` | 2026-07-03 | 2026-07-03 |
| T09 | QA metrics (F1 + EM) | Anish + Bhalchandra | DONE | `src/qa_metrics.py`: SQuAD-normalized token F1 + EM, abstention-aware. **GPT-3.5 baseline:** EM 0.00, F1 0.083 (0.26 on 47 answered) -> `results/eval_qa_metrics.csv` | 2026-07-03 | 2026-07-03 |
| T10 | Preliminary results table | Anish + Bhalchandra | DONE | `src/build_results_table.py` merges RAGAS + F1/EM (all 150, GPT-3.5) -> `results/preliminary_results.md` + `results/results_table.tex` with interpretation; table wired into `paper/main.tex` | 2026-07-03 | 2026-07-03 |
